...
export const projects: Project[] = [
	{
		title: 'PDF2LaTeX',
		description:
			'Transforms N PDFs into a single, organic and complete <strong>LaTeX</strong> document through an agentic <strong>LangGraph</strong> pipeline with a modern, minimal React UI. Inspired by the EPUB-Translator workflow.<br><strong>Agentic pipeline (4 agents):</strong><ul><li><strong>Analyzer</strong> — analyzes each PDF (summary, topics, formulas, figures) in parallel</li><li><strong>Planner</strong> — merges all analyses with the user prompt into a global document structure</li><li><strong>Writer</strong> — writes the LaTeX of each section in parallel</li><li><strong>Reviewer</strong> — fixes compilation errors and inconsistencies, looping with <code>pdflatex</code></li></ul><strong>Stack:</strong> React + Vite + Tailwind frontend ↔ <strong>FastAPI</strong> backend over WebSocket/REST, PyMuPDF extraction, SQLite. Multi-provider (OpenAI, Anthropic, Ollama, custom, fake) with Fernet-encrypted API keys. Docker image bundles TeX Live for <code>pdflatex</code> compilation.',
		techTags: ['LangGraph', 'FastAPI', 'React', 'LaTeX', 'LLM', 'OCR'],
		descriptionIt:
			"Trasforma N PDF in un unico documento <strong>LaTeX</strong> organico e completo tramite una pipeline agentica <strong>LangGraph</strong> con una UI React moderna e minimale. Ispirato al workflow di EPUB-Translator.<br><strong>Pipeline agentica (4 agenti):</strong><ul><li><strong>Analyzer</strong> — analizza ogni PDF (riassunto, argomenti, formule, figure) in parallelo</li><li><strong>Planner</strong> — unisce tutte le analisi con il prompt utente in una struttura globale del documento</li><li><strong>Writer</strong> — scrive il LaTeX di ogni sezione in parallelo</li><li><strong>Reviewer</strong> — corregge errori di compilazione e incoerenze, in loop con <code>pdflatex</code></li></ul><strong>Stack:</strong> frontend React + Vite + Tailwind ↔ backend <strong>FastAPI</strong> via WebSocket/REST, estrazione PyMuPDF, SQLite. Multi-provider (OpenAI, Anthropic, Ollama, custom, fake) con chiavi API cifrate con Fernet. L'immagine Docker include TeX Live per la compilazione <code>pdflatex</code>.",
		githubUrl: 'https://github.com/GiuseppeBellamacina/PDF2Latex',
		image: '/assets/projects/pdf2latex.webp'
	},
	{
	...
	},
	{
		...
		description:
			'Full-stack app that translates entire <strong>EPUB</strong> e-books using LLMs, with a modern React UI and an agentic engine that keeps terminology consistent across the whole book.<br><strong>Agentic pipeline (3 agents):</strong><ul><li><strong>Context Analyzer</strong> — detects genre, tone, characters and key terms</li><li><strong>Translator</strong> — glossary-aware and context-aware translation</li><li><strong>Reviewer</strong> — consistency and glossary-violation check, with automatic retry</li></ul><strong>Key features:</strong> auto-generated editable glossary shared across chapters · smart hybrid paragraph + sliding-window chunking with context overlap · real-time WebSocket progress · side-by-side original/translation preview · translation history with re-download · spacing normalization on rebuilt EPUBs.<br><strong>Stack:</strong> React + Vite + Tailwind ↔ <strong>FastAPI</strong> + LangChain + SQLite. Multi-provider (OpenAI, Anthropic, Ollama or any OpenAI-compatible API). Docker Compose deployable.',
		techTags: ['EPUB', 'LangChain', 'FastAPI', 'React', 'LLM'],
		descriptionIt:
			'App full-stack che traduce interi e-book <strong>EPUB</strong> usando LLM, con una UI React moderna e un motore agentico che mantiene la terminologia coerente in tutto il libro.<br><strong>Pipeline agentica (3 agenti):</strong><ul><li><strong>Context Analyzer</strong> — rileva genere, tono, personaggi e termini chiave</li><li><strong>Translator</strong> — traduzione glossary-aware e context-aware</li><li><strong>Reviewer</strong> — controllo di coerenza e violazioni del glossario, con retry automatico</li></ul><strong>Funzionalità chiave:</strong> glossario editabile auto-generato e condiviso tra i capitoli · chunking ibrido intelligente paragrafo + sliding window con sovrapposizione di contesto · progresso in tempo reale via WebSocket · anteprima affiancata originale/traduzione · cronologia delle traduzioni con ri-download · normalizzazione degli spazi negli EPUB ricostruiti.<br><strong>Stack:</strong> React + Vite + Tailwind ↔ <strong>FastAPI</strong> + LangChain + SQLite. Multi-provider (OpenAI, Anthropic, Ollama o qualsiasi API compatibile OpenAI). Deploy con Docker Compose.',
...
export function getProjects(lang: Lang): Project[] {
	if (lang === 'it') {
		return projects.map((p) => ({
			...p,
			description: p.descriptionIt ?? p.description
		}));
	}
	return projects;
}
